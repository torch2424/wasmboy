...
export function renderSprites(
  ...
  for (let i: i32 = 39; i >= 0; i--) {
    ...
    let spriteYPosition: i32 = eightBitLoadFromGBMemory(
      Graphics.memoryLocationSpriteAttributesTable + spriteTableIndex
    );
    let spriteXPosition: i32 = eightBitLoadFromGBMemory(
      Graphics.memoryLocationSpriteAttributesTable + spriteTableIndex + 1
    );
    let spriteTileId: i32 = eightBitLoadFromGBMemory(
      Graphics.memoryLocationSpriteAttributesTable + spriteTableIndex + 2
    );
    ...
    if (
      scanlineRegister >= spriteYPosition &&
      scanlineRegister < spriteYPosition + spriteHeight
    ) {
      // Then we need to draw the current sprite

      // Get our sprite attributes since we know we shall be drawing the tile
      let spriteAttributes: i32 = eightBitLoadFromGBMemory(
        Graphics.memoryLocationSpriteAttributesTable + spriteTableIndex + 3
      );

      // Check sprite Priority
      let isSpritePriorityBehindWindowAndBackground: boolean = checkBitOnByte(
        7,
        spriteAttributes
      );
      ...
      let spriteTileAddressStart: i32 = getTileDataAddress(
      ...
      let spriteDataByteOneForLineOfTilePixels: i32 = loadFromVramBank(
        spriteTileAddress,
        vramBankId
      );
      let spriteDataByteTwoForLineOfTilePixels: i32 = loadFromVramBank(
        spriteTileAddress + 1,
        vramBankId
      );
      ...
      for (let tilePixel: i32 = 7; tilePixel >= 0; tilePixel--) {
        ...
        if (
          checkBitOnByte(
            spritePixelXInTile,
            spriteDataByteTwoForLineOfTilePixels
          )
        ) {
        ...
        if (
          checkBitOnByte(
            spritePixelXInTile,
            spriteDataByteOneForLineOfTilePixels
          )
        ) {
          spriteColorId += 1;
        }
        ...
        if (spriteColorId !== 0) {
          ...
          let spriteXPixelLocationInCameraView: i32 =
            spriteXPosition + (7 - tilePixel);
          if (
            spriteXPixelLocationInCameraView >= 0 &&
            spriteXPixelLocationInCameraView <= 160
          ) {
            ...
            if (!shouldShowFromLcdcPriority) {
              ...
              let bgPriorityByte: i32 = getPriorityforPixel(
                spriteXPixelLocationInCameraView,
                scanlineRegister
              );

              // Doing an else if, since either will automatically stop drawing the pixel
              if (
                isSpritePriorityBehindWindowAndBackground &&
                (bgPriorityByte & 0x03) > 0
              ) {
                // OAM Priority
            ...
            if (
              shouldShowFromLcdcPriority ||
              (!shouldHideFromOamPriority && !shouldHideFromBgPriority)
            ) {
              if (!Cpu.GBCEnabled) {
                // Get our monochrome color RGB from the current sprite pallete
                // Get our sprite pallete
                let spritePaletteLocation: i32 =
                  Graphics.memoryLocationSpritePaletteOne;
                if (checkBitOnByte(4, spriteAttributes)) {
                  spritePaletteLocation =
                    Graphics.memoryLocationSpritePaletteTwo;
                }
                let spritePixelColorFromPalette: i32 = getMonochromeColorFromPalette(
                  spriteColorId,
                  spritePaletteLocation
                );
