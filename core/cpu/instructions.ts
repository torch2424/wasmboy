...
export function addARegister(register: u8): void {
  ...
  Cpu.registerA = u8Overflow(Cpu.registerA + register);
  if (Cpu.registerA === 0) {
    setZeroFlag(1);
  } else {
    setZeroFlag(0);
...
export function addAThroughCarryRegister(register: u8): void {
  ...
  // CTRL+F adc
  let result: u8 = u8Overflow(Cpu.registerA + register + getCarryFlag());
  if ((u8Overflow(Cpu.registerA ^ register ^ result) & 0x10) != 0) {
    setHalfCarryFlag(1);
  } else {
    setHalfCarryFlag(0);
  }

  let overflowedResult: u16 = u16Overflow(<u16>Cpu.registerA + <u16>register + <u16>getCarryFlag());
...
export function subAThroughCarryRegister(register: u8): void {
  ...
  let carryRegisterCheck = u8Overflow((Cpu.registerA ^ register ^ result) & 0x10);
...
export function shiftLeftRegister(register: u8): u8 {
  ...
  register = u8Overflow(register << 1);
...
export function swapNibblesOnRegister(register: u8): u8 {
  ...
  register = u8Overflow((lowNibble << 4) | (highNibble >> 4));
...
export function relativeJump(value: u8): void {
  ...
  Cpu.programCounter = u16Overflow(Cpu.programCounter + relativeJumpOffset);
