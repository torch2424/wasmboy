    ...
    if(data.action === 'picked') {
      ...
      fetch('https://www.googleapis.com/drive/v2/files/' + googlePickerFileObject.id, {
        ...
        fetch(responseJson.downloadUrl, {
          ...
          this.props.wasmboy._getGameFromArrayBuffer(googlePickerFileObject.name, bytes).then((byteArray) => {
            this.props.wasmboy.loadGame(byteArray)
            .then(() => {
        			console.log('Wasmboy Ready!');
        			this.props.showNotification('Game Loaded! 🎉');
              this.setFileLoadingStatus(false);
        ...
        <div class="wasmboy__filePicker__services">
          ...
          <div class="file">
            <label class="file-label">
              <input class="file-input" type="file" accept=".gb, .gbc, .zip" name="resume" onChange={(event) => {this.loadLocalFile(event)}} />
              <span class="file-cta">
          ...
          {/* Google Drive Picker */}
          <GooglePicker clientId={WASMBOY_DEBUGGER_GOOGLE_PICKER_CLIENT_ID}
                      scope={['https://www.googleapis.com/auth/drive.readonly']}
                      onChange={(data) => {this.loadGoogleDriveFile(data)}}
                      onAuthenticate={token => {googlePickerOAuthToken = token}}
                      multiselect={false}
                      navHidden={true}
                      authImmediate={false}
                      viewId={'DOCS'}>
