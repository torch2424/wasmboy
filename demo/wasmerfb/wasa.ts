...
export class Descriptor {
   ...
   */
  fatime(ts: f64): bool {
    return fd_filestat_set_times(this.rawfd, (ts * 1e9) as u64, 0, fstflags.SET_ATIM) === errno.SUCCESS;
  }
   ...
   */
  fmtime(ts: f64): bool {
    return fd_filestat_set_times(this.rawfd, 0, (ts * 1e9) as u64, fstflags.SET_MTIM) === errno.SUCCESS;
  }
  ...
  futimes(atime: f64, mtime: f64): bool {
    return (
      fd_filestat_set_times(this.rawfd, (atime * 1e9) as u64, (mtime * 1e9) as u64, fstflags.SET_ATIM | fstflags.SET_ATIM) === errno.SUCCESS
    );
  }
   ...
   */
  touch(): bool {
    return fd_filestat_set_times(this.rawfd, 0, 0, fstflags.SET_ATIM_NOW | fstflags.SET_MTIM_NOW) === errno.SUCCESS;
  }
   ...
   */
  read(data: Array<u8> = [], chunk_size: usize = 4096): Array<u8> | null {
   ...
   * @param chunk_size chunk size (default: 4096)
   */
  readAll(data: Array<u8> = [], chunk_size: usize = 4096): Array<u8> | null {
    let data_partial_len = chunk_size;
    let data_partial = changetype<usize>(new ArrayBuffer(data_partial_len));
    let iov = changetype<usize>(new ArrayBuffer(2 * sizeof<usize>()));
    store<u32>(iov, data_partial);
...
export class FileSystem {
  ...
  static link(old_path: string, new_path: string): bool {
    ...
    let fd_lookup_flags = lookupflags.SYMLINK_FOLLOW;
    let res = path_link(old_dirfd, fd_lookup_flags, old_path_utf8, old_path_utf8_len, new_dirfd, new_path_utf8, new_path_utf8_len);
  ...
  static symlink(old_path: string, new_path: string): bool {
    ...
    let new_path_utf8_len: usize = String.UTF8.byteLength(new_path);
    let new_path_utf8 = changetype<usize>(String.UTF8.encode(new_path));
    let res = path_symlink(old_path_utf8, old_path_utf8_len, new_dirfd, new_path_utf8, new_path_utf8_len);
  ...
  static rename(old_path: string, new_path: string): bool {
    ...
    let new_path_utf8_len: usize = String.UTF8.byteLength(new_path);
    let new_path_utf8 = changetype<usize>(String.UTF8.encode(new_path));
    let res = path_rename(old_dirfd, old_path_utf8, old_path_utf8_len, new_dirfd, new_path_utf8, new_path_utf8_len);
...
export class Environ {
  ...
  constructor() {
    this.env = [];
    let count_and_size = changetype<usize>(new ArrayBuffer(2 * sizeof<usize>()));
    ...
    let env_ptrs = changetype<usize>(new ArrayBuffer((count + 1) * sizeof<usize>()));
...
@global
export function wasi_abort(message: string | null = '', fileName: string | null = '', lineNumber: u32 = 0, columnNumber: u32 = 0): void {
  Console.error(fileName! + ':' + lineNumber.toString() + ':' + columnNumber.toString() + ': error: ' + message!);
  proc_exit(255);
}
