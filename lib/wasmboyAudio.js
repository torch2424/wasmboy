...
const WASMBOY_MEMORY_SOUND_INDEX = 0x033400;
...
class WasmBoyAudioService {
    ...
    playAudio() {
      ...
      if(WasmBoy.wasmInstance.exports.getAudioQueueIndex() < WASMBOY_MIN_NUMBER_OF_SAMPLES) {
        return;
      }
      ...
      if (this.audioPlaytime < audioContextCurrentTime) {
        ...
        let wasmBoyNumberOfSamples = WasmBoy.wasmInstance.exports.getAudioQueueIndex();

        const audioBuffer = this.audioContext.createBuffer(2, wasmBoyNumberOfSamples, WASMBOY_SAMPLE_RATE);
        const leftChannelBuffer = audioBuffer.getChannelData(0);
        const rightChannelBuffer = audioBuffer.getChannelData(1);
        ...
        for (let i =  WASMBOY_MEMORY_SOUND_INDEX; (i -  WASMBOY_MEMORY_SOUND_INDEX) < (wasmBoyNumberOfSamples * 2); i++) {
          ...
          let audioSample = WasmBoy.wasmByteMemory[i];
          ...
          // Clear the value
          WasmBoy.wasmByteMemory[i] = 0;
        }

        // Reset the Audio Queue
        WasmBoy.wasmInstance.exports.resetAudioQueue();
