...
class WasmBoyGraphicsService {
  ...
  initialize(canvasElement, wasmInstance, wasmByteMemory, updateGraphicsCallback) {
  ...
  renderFrame() {
    const renderFrameTask = async () => {
      ...
      for (let y = 0; y < GAMEBOY_CAMERA_HEIGHT; y++) {
        for (let x = 0; x < GAMEBOY_CAMERA_WIDTH; x++) {
          ...
          for (let color = 0; color < 3; color++) {
            rgbColor[color] = this.wasmByteMemory[WASMBOY_CURRENT_FRAME_OUTPUT_LOCATION + pixelStart + color];
          }
      ...
      if (this.updateGraphicsCallback) {
        this.updateGraphicsCallback(imageDataArray);
      }
