...
class WasmBoyGraphicsService {
  constructor() {
    ...
    this.placeholderCanvas = undefined;
    this.placeholderCanvasContext = undefined;
    this.placeholderCanvasImageData = undefined;
  ...
  initialize(canvasElement, wasmInstance, wasmByteMemory) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Using the placeholder canvas method: https://stackoverflow.com/questions/24429830/html5-canvas-how-to-change-putimagedata-scale
        // I read the toDataUrl() is slow, so yeah haha!
        this.placeholderCanvas = document.createElement('canvas');
        this.placeholderCanvas.width = GAMEBOY_CAMERA_WIDTH;
        this.placeholderCanvas.height = GAMEBOY_CAMERA_HEIGHT;
        this.placeholderCanvasContext = this.placeholderCanvas.getContext('2d');
        this.placeholderCanvasImageData = this.placeholderCanvasContext.createImageData(GAMEBOY_CAMERA_WIDTH, GAMEBOY_CAMERA_HEIGHT);
        ...
        this.placeholderCanvasContext.clearRect(0, 0, this.canvasElement.clientWidth, this.canvasElement.clientHeight);
      ...
      requestAnimationFrame(() => {
        ...
        if (imageDataArray) {
          ...
          for(let i = 0; i < imageDataArray.length; i++) {
            this.placeholderCanvasImageData.data[i] = imageDataArray[i];
          }
          this.placeholderCanvasContext.clearRect(0, 0, GAMEBOY_CAMERA_WIDTH, GAMEBOY_CAMERA_HEIGHT);
          this.placeholderCanvasContext.putImageData(this.placeholderCanvasImageData, 0, 0);
          ...
          this.canvasContext.drawImage(this.placeholderCanvas, 0, 0);
        }
