...
const MEMORY_ADDRESSES = {
  ...
  WASMBOY_INTERNAL_STATE: 0x000000,
  GAMEBOY_MEMORY: 0x000400
};
...
const getSaveState = (wasmByteMemory) => {
  ...
  const wasmBoyInternalState = new Uint8Array(MEMORY_ADDRESSES.GAMEBOY_MEMORY - MEMORY_ADDRESSES.WASMBOY_INTERNAL_STATE);
  const gameBoyMemory = new Uint8Array(MEMORY_ADDRESSES.CARTRIDGE_RAM - MEMORY_ADDRESSES.GAMEBOY_MEMORY);

  for (let i = MEMORY_ADDRESSES.WASMBOY_INTERNAL_STATE; i < MEMORY_ADDRESSES.GAMEBOY_MEMORY; i++) {
    wasmBoyInternalState[i] = wasmByteMemory[i];
  }

  for (let i = MEMORY_ADDRESSES.GAMEBOY_MEMORY; i < MEMORY_ADDRESSES.CARTRIDGE_RAM; i++) {
    gameBoyMemory[i - MEMORY_ADDRESSES.GAMEBOY_MEMORY] = wasmByteMemory[i];
  }
...
const loadSaveState = (wasmByteMemory, saveState) => {
  ...
  for (let i = MEMORY_ADDRESSES.WASMBOY_INTERNAL_STATE; i < MEMORY_ADDRESSES.GAMEBOY_MEMORY; i++) {
    wasmByteMemory[i] = saveState.wasmBoyMemory.wasmBoyInternalState[i];
  }

  for (let i = MEMORY_ADDRESSES.GAMEBOY_MEMORY; i < MEMORY_ADDRESSES.CARTRIDGE_RAM; i++) {
    wasmByteMemory[i] = saveState.wasmBoyMemory.gameBoyMemory[i - MEMORY_ADDRESSES.GAMEBOY_MEMORY];
  }

  for (let i = MEMORY_ADDRESSES.CARTRIDGE_RAM; i < saveState.wasmBoyMemory.cartridgeRam.length; i++) {
    wasmByteMemory[i] = saveState.wasmBoyMemory.cartridgeRam[i - MEMORY_ADDRESSES.CARTRIDGE_RAM];
  }
...
class WasmBoyMemoryService {
  ...
  initialize(wasmByteMemory) {
    ...
    if (unloadStorage) {
      ...
      responsePromises.push(this.saveCartridgeRam(header, cartridgeRam));
      ...
      if (saveState) {
        ...
        responsePromises.push(this.saveState(header, saveState));
      }


      console.log('Loading unloaded localStorage data', unloadStorageObject);
