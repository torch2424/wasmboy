...
export default class GbChannelWebAudio {
  ...
  cancelAllAudio(stopCurrentAudio) {
    ...
    for (let i = 0, len = audioSources.length; i < len; i++) {
      const audioSource = audioSources[i];
      if (stopCurrentAudio || audioSource.playTime > this.audioPlaytime) {
  ...
  stopRecording() {
    ...
    const createBufferFromBuffers = buffers => {
      let totalLength = buffers.reduce((acc, { length }) => (acc += length), 0);

      const totalBuffer = new Float32Array(totalLength);
      buffers.reduce((acc, buffer) => {
        totalBuffer.set(buffer, acc);
        acc += buffer.length;
      }, 0);
