  ...
  playAudio(currentFps, allowFastSpeedStretching) {
    const playAudioTask = async () => {
      ...
      for (let i = 0; i < wasmBoyNumberOfSamplesForStereo; i = i + 2) {
        leftChannelBuffer[bufferIndex] = getUnsignedAudioSampleAsFloat(this.wasmByteMemory[i + this.WASMBOY_SOUND_OUTPUT_LOCATION]);
        bufferIndex++;
      }
      ...
      for (let i = 1; i < wasmBoyNumberOfSamplesForStereo; i = i + 2) {
        rightChannelBuffer[bufferIndex] = getUnsignedAudioSampleAsFloat(this.wasmByteMemory[i + this.WASMBOY_SOUND_OUTPUT_LOCATION]);
        bufferIndex++;
      }
