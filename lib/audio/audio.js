...
const WASMBOY_MIN_TIME_REMAINING_IN_SECONDS =
  WASMBOY_MIN_TIME_REMAINING_IN_MILLI / 1000;
...
class WasmBoyAudioService {
  ...
  initialize(wasmInstance, wasmByteMemory) {
    ...
    WASMBOY_SOUND_OUTPUT_LOCATION = this.wasmInstance.exports
      .soundOutputLocation;
  ...
  playAudio(currentFps, allowFastSpeedStretching) {
    return new Promise(resolve => {
      ...
      let audioContextCurrentTimeWithLatency =
        audioContextCurrentTime + DEFAULT_AUDIO_LATENCY_IN_SECONDS;
      this.audioPlaytime =
      ...
      for (let i = 0; i < wasmBoyNumberOfSamplesForStereo; i = i + 2) {
        leftChannelBuffer[bufferIndex] = getUnsignedAudioSampleAsFloat(
          this.wasmByteMemory[i + WASMBOY_SOUND_OUTPUT_LOCATION]
        );
        bufferIndex++;
      }
      ...
      for (let i = 1; i < wasmBoyNumberOfSamplesForStereo; i = i + 2) {
        rightChannelBuffer[bufferIndex] = getUnsignedAudioSampleAsFloat(
          this.wasmByteMemory[i + WASMBOY_SOUND_OUTPUT_LOCATION]
        );
        bufferIndex++;
      }
      ...
      const sourcePlaybackLength =
        wasmBoyNumberOfSamples / (WASMBOY_SAMPLE_RATE * playbackRate);
      ...
      while (
        ...
        this.audioSources[this.audioSources.length - 1].playtime <=
          this.audioPlaytime
      ) {
      ...
      const timeUntilSourceEnds =
        this.audioPlaytime - this.audioContext.currentTime + 500;
      setTimeout(() => {
  ...
  cancelAllAudio() {
    ...
    this.audioPlaytime =
      this.audioContext.currentTime + DEFAULT_AUDIO_LATENCY_IN_SECONDS;
  }
