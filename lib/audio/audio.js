...
class WasmBoyAudioService {
    ...
    playAudio(currentFps) {
      return new Promise((resolve) => {

        // Find our averageFps
        let fps = 60;
        if (currentFps && currentFps > 0 && currentFps < 60) {
          this.averageFps.push(currentFps);
          if (this.averageFps.length > 60) {
            this.averageFps.shift();
          }
          fps = this.averageFps.reduce((accumulator, currentValue) => {
            return accumulator + currentValue;
          });
          fps = Math.floor(fps / this.averageFps.length);
        } else {
          this.averageFps = [];
        }
        ...
        this.audioSources.push({
          ...
          timeStretched: this.timeStretchingEnabled
        });
        while (this.audioSources.length > 5) {
          this.audioSources.shift();
        }

        // Set our new audio playtime goal
        this.audioPlaytime = this.audioPlaytime +
          (wasmBoyNumberOfSamples / (WASMBOY_SAMPLE_RATE * playbackRate));

        if (this.timeStretchingEnabled) {
          this.timeStretchPlaytime = this.audioPlaytime;
        } else {
          // Try to remove any time stretched samples that will start after the current one
          for (let i = 0; i < this.audioSources.length; i++) {
            if (this.audioSources[i].timeStretched &&
                this.audioSources[i].playTime > this.audioPlaytime) {
