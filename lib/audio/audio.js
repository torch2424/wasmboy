...
class WasmBoyAudioService {
  ...
  playAudio(audioMessage) {
    ...
    // Finally play on all of our channels if we have buffers for them
    let playingIndividual = false;
    // Play the individual channels
    for (let i = 0; i < 4; i++) {
    ...
    if (playingIndividual) {
      if (!this.gbChannels.master.muted) {
        this.gbChannels.master.mute();
      }

      if (this.gbChannels.channel1.muted) {
        this.gbChannels.channel1.unmute();
      }
      if (this.gbChannels.channel2.muted) {
        this.gbChannels.channel2.unmute();
      }
      if (this.gbChannels.channel3.muted) {
        this.gbChannels.channel3.unmute();
      }
      if (this.gbChannels.channel4.muted) {
        this.gbChannels.channel4.unmute();
      }
    } else if (!playingIndividual) {
      if (this.gbChannels.master.muted) {
        this.gbChannels.master.unmute();
      }

      if (!this.gbChannels.channel1.muted) {
        this.gbChannels.channel1.mute();
      }
      if (!this.gbChannels.channel2.muted) {
        this.gbChannels.channel2.mute();
      }
      if (!this.gbChannels.channel3.muted) {
        this.gbChannels.channel3.mute();
      }
      if (!this.gbChannels.channel4.muted) {
        this.gbChannels.channel4.mute();
      }
    }
  }
