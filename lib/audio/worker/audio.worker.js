  ...
  switch (eventData.message.type) {
    ...
    case WORKER_MESSAGE_TYPE.UPDATED: {
      ...
      audioDebuggingChannelBufferKeys.forEach(channelBufferKey => {
        ...
        const logChannelBuffer = 'channel3Buffer';
        if (channelBufferKey === logChannelBuffer) {
          // Count the values in the buffer
          const valueCount = {};
          audioBufferAsArray.forEach(value => {
            if (valueCount[value] === undefined) {
              valueCount[value] = 0;
            }
            valueCount[value]++;
          });
          console.log(logChannelBuffer + ' audio buffer', valueCount);

          const valueCountLeft = {};
          new Float32Array(audioChannelBuffers.left).forEach(value => {
            if (valueCountLeft[value] === undefined) {
              valueCountLeft[value] = 0;
            }
            valueCountLeft[value]++;
          });
          console.log(logChannelBuffer + ' left audio buffer', valueCountLeft);

          const valueCountRight = {};
          new Float32Array(audioChannelBuffers.right).forEach(value => {
            if (valueCountRight[value] === undefined) {
              valueCountRight[value] = 0;
            }
            valueCountRight[value]++;
          });
          console.log(logChannelBuffer + ' right audio buffer', valueCountRight);
        }
