...
const getAudioChannelsFromBuffer = (audioBuffer, numberOfSamples) => {
  // Create our buffers as Float 32 Array
  // https://developer.mozilla.org/en-US/docs/Web/API/AudioBuffer/getChannelData
  const leftChannelBuffer = new Float32Array();
  const rightChannelBuffer = new Float32Array();
...
const libMessageHandler = event => {
  ...
  switch (eventData.message.type) {
    ...
    case WORKER_MESSAGE_TYPE.UPDATED: {
      ...
      const audioChannels = getAudioChannelsFromBuffer(
        new Uint8ClampedArray(eventData.message.audioBuffer),
        eventData.message.numberOfSamples
      );
      postMessage(
        getSmartWorkerMessage({
          type: WORKER_MESSAGE_TYPE.UPDATED,
          leftChannel: audioChannels.left,
          rightChannel: audioChannels.right,
          numberOfSamples: eventData.message.numberOfSamples
        }),
        [audioChannels.left, audioChannels.right]
      );
      return;
    }
  }
