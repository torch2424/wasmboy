...
class WasmBoyLib {
  ...
  constructor() {
    ...
    this.leftChannelBuffer = undefined;
    this.rightChannelBuffer = undefined;
  ...
  _audio() {
    ...
    let wasmBoyNumberOfSamples = wasmBoySampleRate;
    // Samples per frame, * 2 for stereo
    let wasmBoyNumberOfStereoSamplesPerFrame = (wasmBoySampleRate / 60) * 2;

    // Set our soundIndex in wasmboy memory
    let soundIndex = 0x033400;

    // Re-initialize our buffers
    if(this.currentFrame === 0) {
      this.audioBuffer = this.audioContext.createBuffer(2, wasmBoyNumberOfSamples, wasmBoySampleRate);
      this.bufferIndex = 0;
      this.leftChannelBuffer = this.audioBuffer.getChannelData(0);
      this.rightChannelBuffer = this.audioBuffer.getChannelData(1);
    }

    // Iterate through our samples
    let i;
    for (i = soundIndex; (i - soundIndex) < wasmBoyNumberOfStereoSamplesPerFrame && this.wasmByteMemory[i] !== 0; i++) {

      // Get our audio sample
      let audioSample = this.wasmByteMemory[i];

      //console.log('original', audioSample);

      // Subtract 1 as it is added so the value is not empty
      audioSample -= 1;
      // Subtract 7 as that is our 0 value (for now in testing), since we want -1.0 to 1.0
      audioSample -= 7;
      // Divide by 7 to get our sample as a decimal
      audioSample = audioSample / 10;
      ...
      if((i - soundIndex) % 2 == 0) {
        this.leftChannelBuffer[this.bufferIndex] = audioSample;
      } else {
        //this.rightChannelBuffer[this.bufferIndex] = audioSample;
        this.bufferIndex += 1;
      }
    ...
    // Having trouble writing to memory and grabbing it
    console.log(i - soundIndex, this.bufferIndex, this.currentFrame, this.wasmByteMemory[i] !== 0);

    // Check if our buffer is filled and we should play it
    if(this.currentFrame === 60) {

      // Output Nothing
      // for(let i = 0; i < this.leftChannelBuffer.length; i++) {
      //   this.leftChannelBuffer[i] = -0.7
      // }

      // Output a noise
      // for(let i = 0; i < this.leftChannelBuffer.length; i++) {
      //   this.leftChannelBuffer[i] = i % 100 < 50 ? -0.7 : 0.7
      // }

      console.log(this.leftChannelBuffer);
      ...
      // Get our playtime
      // See: https://github.com/binji/binjgb/blob/master/demo/demo.js
      // To avoid having to watch audio started
      if(this.audioStarted) {
        this.audioPlaytime = this.audioPlaytime + (wasmBoyNumberOfSamples / wasmBoySampleRate);
      } else {
        this.audioPlaytime = this.audioContext.currentTime + 1.5;
