...
class WasmBoyLib {
  ...
  loadGame(game) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pauseGame().then(() => {
        ...
        if (
          !this.headless &&
          WasmBoyMemory.getLoadedCartridgeMemoryState().RAM
        ) {
        ...
        Promise.all(initPromises)
          .then(responses => {
            ...
            if (this.headless) {
              WasmBoyMemory.initializeHeadless(
                this.wasmInstance,
                this.wasmByteMemory,
              ...
              Promise.all([
                ...
                .then(() => {
                  ...
                  // TODO: Handle passing a boot rom
                  WasmBoyMemory.loadCartridgeRom(
                    responses[0],
                    this.isGbcEnabled,
                    false
                  );
  ...
  _emulatorUpdate() {
    ...
    if (
      ...
      this.isAudioEnabled &&
      this.wasmInstance.exports.getAudioQueueIndex() >
        6000 * (this.gameboyFrameRate / 120) &&
      this.gameboyFrameRate <= 60
    ) {
    ...
    if (response > 0) {
      ...
      console.log(
        `Program Counter: 0x${this.wasmInstance.exports
          .getProgramCounter()
          .toString(16)}`
      );
      console.log(
        `Opcode: 0x${this.wasmByteMemory[
  ...
  _getWasmInstance() {
    return new Promise((resolve, reject) => {
      ...
      const memoryBase = WebAssembly.instantiateStreaming(
        fetch(wasmModuleUrl),
        {
          env: {
            log: (message, arg0, arg1, arg2, arg3, arg4, arg5) => {
              // Grab our string
              var len = new Uint32Array(
            ...
            performanceTimestamp: (id, value) => {
              ...
              if (!this.performanceTimestamps[id].throttle) {
                if (this.performanceTimestamps[id].timestamp) {
                  ...
                  let timeDifference =
                    endTime - this.performanceTimestamps[id].timestamp;
                  ...
                  console.log(
                    `[WasmBoy] Performance Timestamp. ID: ${id}, Time: ${timeDifference}, value difference: ${value -
                      this.performanceTimestamps[id].value}, total time: ${
                      this.performanceTimestamps[id].totalTime
                    }`
                  );
                  ...
                  this.performanceTimestamps[
                    id
                  ].timestamp = getPerformanceTimestamp();
        ...
        .then(instantiatedWasm => {
          ...
          const instance = (this.wasmInstance =
            instantiatedWasm.instance || instantiatedWasm);
          ...
          // Will stay in sync
          this.wasmByteMemory = new Uint8Array(
            this.wasmInstance.exports.memory.buffer
          );
  ...
  _fetchGameAsByteArray(game) {
    return new Promise((resolve, reject) => {
      if (ArrayBuffer.isView(game) && game.constructor === Uint8Array) {
        ...
        fileReader.onload = () => {
          this._getGameFromArrayBuffer(game.name, fileReader.result)
            .then(byteArray => {
              resolve(byteArray);
            })
            .catch(error => {
        ...
        fetch(game)
          ...
          .then(bytes => {
            this._getGameFromArrayBuffer(game, bytes)
              .then(byteArray => {
                resolve(byteArray);
              })
              .catch(error => {
  ...
  _getGameFromArrayBuffer(fileName, gameBuffer) {
    return new Promise((resolve, reject) => {
      if (fileName.endsWith(".zip")) {
        const JSZip = require("jszip");
        JSZip.loadAsync(gameBuffer).then(
          zip => {
            ...
            zip.forEach((relativePath, zipEntry) => {
              if (!foundGame) {
                if (
                  relativePath.endsWith(".gb") ||
                  relativePath.endsWith(".gbc")
                ) {
            ...
            if (!foundGame) {
              reject(
                new Error('The ".zip" did not contain a ".gb" or ".gbc" file!')
              );
            }
          },
          error => {
        ...
        const byteArray = new Uint8Array(gameBuffer);
        resolve(byteArray);
      }
    });
  }
