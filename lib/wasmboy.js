...
class WasmBoyLib {
  ...
  _getWasmInstance() {
    return new Promise((resolve, reject) => {
      ...
      const memoryBase =
      WebAssembly.instantiateStreaming(fetch(wasmModuleUrl), {
        env: {
  ...
  _fetchGameAsByteArray(game) {
    return new Promise((resolve, reject) => {
      if (ArrayBuffer.isView(game) && game.constructor === Uint8Array) {
        ...
        fileReader.onload = () => {
          this._getGameFromArrayBuffer(game.name, fileReader.result).then((byteArray) => {
            resolve(byteArray);
          }).catch((error) => {
            reject(error);
          });
        ...
        .then(blob => {
          ...
          this._getGameFromArrayBuffer(game, bytes).then((byteArray) => {
            resolve(byteArray);
          }).catch((error) => {
            reject(error);
          });
          ...
          zip.forEach((relativePath, zipEntry) => {
            if(!foundGame) {
              if(relativePath.endsWith('.gb') || relativePath.endsWith('.gbc')) {
                ...
                foundGame = true;
                zip.file(relativePath).async('uint8array').then((gameBuffer) => {
                    resolve(gameBuffer);
                })
              }
            }
