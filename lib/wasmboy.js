...
class WasmBoyLib {
  ...
  loadGame(game) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pauseGame().then(() => {
        // Get our promises
        const initPromises = [this._fetchGameAsByteArray(game), this._getWasmInstance()];
  ...
  _emulatorUpdate() {
    ...
    if (
      ...
      this.isAudioEnabled &&
      this.wasmInstance.exports.getAudioQueueIndex() > 6000 * (this.gameboyFrameRate / 120) &&
      this.gameboyFrameRate <= 60
    ) {
  ...
  _getWasmInstance() {
    return new Promise((resolve, reject) => {
      ...
      const memoryBase = WebAssembly.instantiateStreaming(fetch(wasmModuleUrl), {
        env: {
              ...
              if (this.performanceTimestamps[id].timestamp) {
                ...
                console.log(
                  `[WasmBoy] Performance Timestamp. ID: ${id}, Time: ${timeDifference}, value difference: ${value -
                    this.performanceTimestamps[id].value}, total time: ${this.performanceTimestamps[id].totalTime}`
                );
