...
class WasmBoyLib {
  ...
  constructor() {
    ...
    this.wasmModuleRequest = undefined;
  ...
  loadGame(pathToGame) {
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.all(initPromises).then((responses) => {
        ...
        // requestAnimationFrame to ensure the game is not running while we paused
    ...
    return new Promise((resolve, reject) => {
      ...
      this.wasmInstance = this.wasmModuleRequest.then((binary) => {
        WebAssembly.instantiate(binary, {
          ...
          console.log('[WasmBoy] Module Instantiated!', instance);
  ...
  _fetchGameAsByteArray(pathToGame) {
    return new Promise((resolve, reject) => {
      ...
      if(typeof pathToGame === 'object' && pathToGame.size) {
        // Read the file
        ...
        fileReader.readAsArrayBuffer(pathToGame);
      } else {
        // Fetch the file
        fetch(pathToGame)
        .then(blob => {
