...
class WasmBoyLib {
  ...
  _audio() {
    ...
    // Create an empty buffer with a the length of one frame (sampleRate / 60) at the sample rate of the AudioContext
    ...
    for (let i = soundIndex; this.wasmByteMemory[i] !== 0; i += 2) {
      ...
      audioSample -= 7;
      // Divide by 100 to get our sample as a decimal
      audioSample = audioSample / 100;

      leftChannelBuffer[i - soundIndex] = audioSample;
    ...
    for (let i = soundIndex + 1; this.wasmByteMemory[i] !== 0; i += 2) {
      ...
      rightChannelBuffer[i - soundIndex - 1] = audioSample;
