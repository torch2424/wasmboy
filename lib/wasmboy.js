...
class WasmBoyLib {
  ...
  initialize(canvasElement, wasmBoyOptions) {
    ...
    if(wasmBoyOptions) {
      ...
      // Headless mode
      if(wasmBoyOptions.headless) {
        this.headless = true;
      }

      // Frameskip
      if (wasmBoyOptions.frameSkip && wasmBoyOptions.frameSkip > 0) {
        this.frameSkip = wasmBoyOptions.frameSkip;
      }

      // Disable Audio
      if (wasmBoyOptions.enableAudio) {
        this.isAudioEnabled = wasmBoyOptions.enableAudio;
      }

      // Gameboy Speed / Framerate
      if (wasmBoyOptions.gameboySpeed) {
        let gameboyFrameRate = Math.floor(60 * wasmBoyOptions.gameboySpeed);
        if(gameboyFrameRate <= 0) {
          gameboyFrameRate = 1;
        }
        this.gameboyFrameRate = gameboyFrameRate;
      }
      if(wasmBoyOptions.gameboyFrameRate) {
        let gameboyFrameRate = wasmBoyOptions.gameboyFrameRate;
        if(gameboyFrameRate <= 0) {
          gameboyFrameRate = 1;
        }
        this.gameboyFrameRate = gameboyFrameRate;
