...
const getSaveState = (wasmInstance, wasmByteMemory, saveStateCallback, saveStateName) => {
  ...
  wasmInstance.exports.saveState();
  ...
  for (let i = 0; i < WASMBOY_PALETTE_MEMORY_SIZE; i++) {
    wasmBoyPaletteMemory[i] = wasmByteMemory[i + WASMBOY_PALETTE_MEMORY_LOCATION];
  }
...
const loadSaveState = (wasmInstance, wasmByteMemory, saveState) => {
  ...
  for (let i = 0; i < WASMBOY_PALETTE_MEMORY_SIZE; i++) {
    wasmByteMemory[i + WASMBOY_PALETTE_MEMORY_LOCATION] = saveState.wasmBoyMemory.wasmBoyPaletteMemory[i];
  }

  for (let i = 0; i < WASMBOY_INTERNAL_MEMORY_SIZE; i++) {
    wasmByteMemory[i + WASMBOY_INTERNAL_MEMORY_LOCATION] = saveState.wasmBoyMemory.gameBoyMemory[i];
  }

  for (let i = 0; i < saveState.wasmBoyMemory.cartridgeRam.length; i++) {
    wasmByteMemory[i + WASMBOY_GAME_RAM_BANKS_LOCATION] = saveState.wasmBoyMemory.cartridgeRam[i];
  }
...
class WasmBoyMemoryService {
  ...
  initialize(wasmInstance, wasmByteMemory, saveStateCallback) {
    ...
    const unloadHandler = () => {
      ...
      if (!this.wasmInstance || this.wasmInstance.exports.hasCoreStarted() <= 0) {
        return null;
      }
