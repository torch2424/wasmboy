...
class WasmBoyMemoryService {
      ...
      switch (eventData.message.type) {
        case WORKER_MESSAGE_TYPE.UPDATED: {
          ...
          if (memoryKeys.includes(MEMORY_TYPE.CARTRIDGE_ROM)) {
            this.cartridgeRom = eventData.message[MEMORY_TYPE.CARTRIDGE_ROM];
          }
    ...
    const loadTask = async () => {
      ...
      await this.worker
        ...
        .then(event => {
          ...
          this.cartridgeRom = new Uint8Array(ROM.buffer);
        });
      ...
      await this.worker
        .postMessage({
          ...
          memoryTypes: [MEMORY_TYPE.CARTRIDGE_HEADER]
        })
        .then(event => {
          const eventData = getEventData(event);
          this.cartridgeHeader = eventData.message[MEMORY_TYPE.CARTRIDGE_HEADER];
        });
    };
  ...
  loadCartridgeRam() {
    const loadCartridgeRamTask = async () => {
      ...
      const workerMemoryObject = {};
      workerMemoryObject[MEMORY_TYPE.CARTRIDGE_RAM] = cartridgeObject.cartridgeRam;
      await this.worker
        .postMessage(
          {
            type: WORKER_MESSAGE_TYPE.SET_MEMORY,
  ...
  loadState(saveState) {
    const loadStateTask = async () => {
      ...
      const workerMemoryObject = {};
      workerMemoryObject[MEMORY_TYPE.CARTRIDGE_RAM] = saveState.wasmboyMemory.cartridgeRam;
      workerMemoryObject[MEMORY_TYPE.GAMEBOY_MEMORY] = saveState.wasmboyMemory.gameBoyMemory;
      workerMemoryObject[MEMORY_TYPE.PALETTE_MEMORY] = saveState.wasmboyMemory.wasmBoyPaletteMemory;
      workerMemoryObject[MEMORY_TYPE.INTERNAL_STATE] = saveState.wasmboyMemory.wasmBoyInternalState;
      ...
      await this.worker.postMessage(
        ...
        [
          saveState.wasmboyMemory.cartridgeRam,
          saveState.wasmboyMemory.gameBoyMemory,
          saveState.wasmboyMemory.wasmBoyPaletteMemory,
          saveState.wasmboyMemory.wasmBoyInternalState
        ]
      );
