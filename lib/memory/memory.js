...
const getCartridgeRam = (wasmInstance, wasmByteMemory) => {
  ...
  // http://gbdev.gg8.se/wiki/articles/The_Cartridge_Header
  let cartridgeType = wasmByteMemory[wasmInstance.exports.gameBytesLocation + 0x0147];
...
const getSaveState = (wasmInstance, wasmByteMemory) => {
  // Simply read up to: 0x0083FF,
  // then append our catridge ram
  const cartridgeRam = getCartridgeRam(wasmByteMemory);

  const wasmBoyInternalState = new Uint8Array(wasmInstance.exports.wasmBoyInternalStateSize);
  const gameBoyMemory = new Uint8Array(wasmInstance.exports.gameBoyInternalMemorySize);

  for (let i = 0; i < wasmInstance.exports.wasmBoyInternalStateSize; i++) {
    wasmBoyInternalState[i] =
      wasmByteMemory[i + wasmInstance.exports.wasmBoyInternalStateLocation];
  }

  for (let i = 0; i < wasmInstance.exports.gameBoyInternalMemorySize; i++) {
    gameBoyMemory[i] =
      wasmByteMemory[i + wasmInstance.exports.gameBoyInternalMemoryLocation];
  }
...
const loadSaveState = (wasmInstance, wasmByteMemory, saveState) => {
  ...
  for (let i = 0; i < saveState.wasmBoyMemory.cartridgeRam.length; i++) {
    wasmByteMemory[i + wasmInstance.exports.gameRamBanksLocation] =
...
class WasmBoyMemoryService {
  ...
  initialize(wasmInstance, wasmByteMemory, includeBootRom) {
    ...
    window.addEventListener("beforeunload", () => {
      ...
      const saveState = getSaveState(this.wasmInstance, this.wasmByteMemory);
  ...
  resetMemory() {
    for(let i = 0; i <= this.wasmInstance.exports.gameBytesLocation; i++) {
      this.wasmByteMemory[i] = 0;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      idbKeyval.get(header).then((cartridgeObject) => {
        ...
        for(let i = 0; i < cartridgeObject.cartridgeRam.length; i++) {
           this.wasmByteMemory[this.wasmInstance.exports.gameRamBanksLocation + i] = cartridgeObject.cartridgeRam[i];
        }
  ...
  saveState(passedHeader, passedSaveState) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (passedHeader && passedSaveState) {
        ...
        saveState = getSaveState(this.wasmInstance, this.wasmByteMemory);
        header = getCartridgeHeader(this.wasmInstance, this.wasmByteMemory);
      }
  ...
  loadState(saveStateIndex) {
    return new Promise((resolve, reject) => {
      ...
      idbKeyval.get(header).then((cartridgeObject) => {
        ...
        loadSaveState(this.wasmInstance, this.wasmByteMemory, cartridgeObject.saveStates[saveStateIndex]);
