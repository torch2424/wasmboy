...
const passMemoryToPort = (postMessageHandler, eventData) => {
  ...
  postMessageHandler(getSmartWorkerMessage(eventData.message, eventData.messageId), transferableMemory);
};
...
const libMessageHandler = event => {
  ...
  switch (eventData.message.type) {
    ...
    case WORKER_MESSAGE_TYPE.GET_MEMORY: {
      passMemoryToPost(postMessage, eventData);
      return;
    }
  }
};
...
const messageHandler = event => {
  ...
  switch (eventData.message.type) {
    ...
    case WORKER_MESSAGE_TYPE.SET_MEMORY: {
      passMemoryToPort(libWorkerPort.postMessage, eventData);
      return;
    }
