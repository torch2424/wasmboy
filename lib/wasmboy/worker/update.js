...
export function update(libWorker, intervalRate) {
  ...
  if (response >= 0) {
    ...
    const memoryBasedWorkerPorts = [libWorker.graphicsWorkerPort, libWorker.memoryWorkerPort, libWorker.audioWorkerPort];
    memoryBasedWorkerPorts.forEach(workerPort => {
      // Can't wrap in object, must be straight for performance
      workerPort.postMessage(libWorker.wasmByteMemory.buffer[libWorker.wasmByteMemory.buffer]);
    });
