      ...
      var memoryBase = index_untouched_default()({
        env: {
          ...
          performanceTimestamp: function performanceTimestamp(id, value) {
            if (!_this6.performanceTimestamps[id]) {
              _this6.performanceTimestamps[id] = {};
              _this6.performanceTimestamps[id].throttle = false;
              _this6.performanceTimestamps[id].totalTime = 0;
              _this6.performanceTimestamps[id].value = 0;
            }
            if (!_this6.performanceTimestamps[id].throttle) {
              if (_this6.performanceTimestamps[id].timestamp) {
                // sleep a millisecond for hopefully more accurate times
                var endTime = performance.now();
                var timeDifference = endTime - _this6.performanceTimestamps[id].timestamp;
                _this6.performanceTimestamps[id].throttle = true;
                _this6.performanceTimestamps[id].totalTime += timeDifference;
                console.log('[WasmBoy] Performance Timestamp. ID: ' + id + ', Time: ' + timeDifference + ', value difference: ' + (value - _this6.performanceTimestamps[id].value) + ', total time: ' + _this6.performanceTimestamps[id].totalTime);
                _this6.performanceTimestamps[id].timestamp = false;
                setTimeout(function () {
                  _this6.performanceTimestamps[id].throttle = false;
                }, 100);
              } else {
                _this6.performanceTimestamps[id].timestamp = performance.now();
                _this6.performanceTimestamps[id].value = value;
              }
            }
          }
        }
